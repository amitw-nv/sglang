    ...
    def maybe_register_and_publish_weight_info(self) -> None:
        if (
            ...
            if self._nixl_agent is not None:
                # NIXL path: register VRAM regions, then capture agent metadata.
                # Metadata must be captured AFTER registration so the blob includes
                # the rkeys Miles needs to RDMA-WRITE into the weight buffers.
                ...
                self.server_args.remote_instance_weight_loader_backend
