...
class ScriptedHttpServer:
    ...
    def _await_http_ready(self) -> None:
        ...
        # entrypoint may not be accepting connections yet. The first script runs
        # _reset_engine_state, which POSTs to this server's own HTTP port, so
        # block until /health returns 200 (server past the Starting state and
        # the socket is bound) before any script can run.
        ...
        raise TimeoutError(
            f"ScriptedHttpServer: HTTP endpoint {url} not ready within "
